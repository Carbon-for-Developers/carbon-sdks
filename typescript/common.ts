...
import { readableStreamToString, CarbonError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new CarbonError(e, parseIfJson(responseBody), e.response?.headers)
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new CarbonError(e, e.response?.data, e.response?.headers)
                }
                if (innerError instanceof CarbonError) {
                    // Got "CarbonError" from the above try block
                    throw innerError;
                }
